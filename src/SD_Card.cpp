...
void SD_TEST() {
  SD_TEST_listDir(SD, "/", 0);
  SD_TEST_createDir(SD, "/mydir");
  SD_TEST_listDir(SD, "/", 0);
  SD_TEST_removeDir(SD, "/mydir");
  SD_TEST_listDir(SD, "/", 2);
  SD_TEST_writeFile(SD, "/hello.txt", "Hello ");
  SD_TEST_appendFile(SD, "/hello.txt", "World!\n");
  SD_TEST_readFile(SD, "/hello.txt");
  SD_TEST_deleteFile(SD, "/foo.txt");
  SD_TEST_renameFile(SD, "/hello.txt", "/foo.txt");
  SD_TEST_readFile(SD, "/foo.txt");
  SD_TEST_testFileIO(SD, "/test.txt");
  Serial.printf("Total space: %llu MB\n", SD.totalBytes() / (1024 * 1024));
  Serial.printf("Used space: %llu MB\n", SD.usedBytes() / (1024 * 1024));
}
